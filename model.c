...
void convert2prim(double prim[11], double **conserved, int c, double X[3],
    ...
    detgamma = get_detgamma( X[0], X[1], X[2]);
    get_spatial_lc(lc);
    ...
    for (int j = 1; j < 4; j++) {
        for (int i = 1; i < 4; i++) {
            ...
            E_d[j] += g_dd[i][j] * conserved[E1 + i - 1][c];
	...
	for(int j=1;j<NDIM;j++)
		...
		ExB[i] += (detgamma)*lc[i][j][k] * conserved[E1 + j - 1][c] * conserved[B1 + k - 1][c];
           }
...
#if(RESISTIVE)
    prim[UU] = (conserved[XI][c] - conserved[TAU][c] - conserved[D][c] + 0.5 *(Esq+Bsq))/(neqpar[0]-1);
    ...
    U_d[1] = (conserved[S1][c] - ExB[1])/conserved[XI][c];
    U_d[2] = (conserved[S2][c] - ExB[2])/conserved[XI][c];
    U_d[3] = (conserved[S3][c] - ExB[3])/conserved[XI][c];

    for (int j = 1; j < 4; j++) {
        for (int i = 1; i < 4; i++) 
            U_u[j] += gamma[i][j] * U_d[i];
    }
    prim[U1]=U_u[1];
    prim[U2]=U_u[2];
    prim[U3]=U_u[3];
...
//    }
    ...
    prim[E1] =  conserved[E1][c];
    prim[E2] =  conserved[E2][c];
    prim[E3] =  conserved[E3][c];
...
void init_grmhd_data(char *fname) {
    ...
    long int stag =
        nleafs_long * (nx[0] + 1) * (nx[1] + 1) * nws * 8;
...
#endif
                double prim[11];
                convert2prim(prim, values, c, Xgrid[i][c], Xgrid[i][c],
...
void set_units(double M_unit_) {
    ...
    L_unit = 1;//GGRAV * MBH / (SPEED_OF_LIGHT * SPEED_OF_LIGHT);
    T_unit = 1;//L_unit / SPEED_OF_LIGHT;

    RHO_unit = 1;//M_unit_ / pow(L_unit, 3);
    U_unit = 1;//RHO_unit * SPEED_OF_LIGHT * SPEED_OF_LIGHT;
    B_unit = 1;//SPEED_OF_LIGHT * sqrt(4. * M_PI * RHO_unit);

    Ne_unit = 1;//RHO_unit / (PROTON_MASS + ELECTRON_MASS);
}
...
int get_fluid_params(double X[NDIM], struct GRMHD *modvar) {
    ...
    double del[NDIM];
    double rho, uu;
    double Bp[NDIM]={0,0,0,0}, Ep[NDIM]={0,0,0,0}, V_u[NDIM];
...
//        return 0;
    ...
    Ep[1] = interp_scalar(p[E1][igrid], c, del);
    Ep[2] = interp_scalar(p[E2][igrid], c, del);
    Ep[3] = interp_scalar(p[E3][igrid], c, del);
    ...
    double BV = 0;
    double n[NDIM]={1/alpha,-shift[1]/alpha,-shift[2]/alpha,-shift[3]/alpha};
    double lc[NDIM][NDIM][NDIM];
    double VxB[NDIM] ={0,0,0,0};
    double detgam=0;
    detgam = get_detgamma(X[1],X[2],X[3]);
    double V_d[4] = {0,0,0,0};
    double B_d[4] = {0,0,0,0};

    get_full_lc(lc);

    for (i = 1; i < NDIM; i++) {
        for (int l = 1; l < NDIM; l++) {
            V_d[i] += (gamma_dd[i][l] * V_u[l]);
            B_d[i] += (gamma_dd[i][l] * Bp[l]);
        }
    }
    ...
    for (int i = 0; i < NDIM; i++) {
        for (int j = 0; j < NDIM; j++) {
            for (int k = 0; k < NDIM; k++) {
                VxB[i] += 1/sqrt(detgam) * lc[i][j][k] * V_d[j] * B_d[k];
            }
        }
    }

    for (int i = 0; i < NDIM; i++) {
        (*modvar).B_u[i] = lfac/alpha * BV + lfac * ( Ep[i] + VxB[i]);
    }
